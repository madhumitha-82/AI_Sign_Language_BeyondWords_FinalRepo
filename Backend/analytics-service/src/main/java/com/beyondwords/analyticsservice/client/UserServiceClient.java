package com.beyondwords.analyticsservice.client;
...
public class UserServiceClient {
    ...
    /**
     * Fetch all enabled users sorted by XP descending from user-service.
     * Used to build the real leaderboard.
     */
    @SuppressWarnings({"unchecked", "rawtypes"})
    public java.util.List<java.util.Map<String, Object>> getLeaderboardUsers() {
        try {
            ResponseEntity<java.util.List> response = restTemplate.getForEntity(
                    userServiceBaseUrl + "/api/users/internal/leaderboard",
            ...
            log.error("Failed to fetch leaderboard users: {}", e.getMessage());
        }
        return java.util.Collections.emptyList();
    }

    public void awardWeeklyBadge(String userEmail, String badgeType, java.time.LocalDate weekStart, java.time.LocalDate weekEnd) {
        try {
            Map<String, Object> payload = new HashMap<>();
            payload.put("email", userEmail);
            payload.put("badgeType", badgeType);
            ...
            restTemplate.postForEntity(
                    userServiceBaseUrl + "/api/users/internal/badges/award-weekly",
                    payload,
                    Void.class
            );
        } catch (Exception e) {
            log.error("Failed to award weekly badge {} for user {}: {}", badgeType, userEmail, e.getMessage());
        }
    }
}
